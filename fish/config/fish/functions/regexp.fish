function regexp --description "in: .config/fish/functions/echoeach.fish"
  if count $argv >/dev/null
    switch $argv[1]
      case -h -he -hel -help  --h --he --hel --help
        set -l subs
        set -l subs $subs '\s' '[[:space:]]'
        set -l subs $subs '\S' '[^[:space:]]'
        set -l subs $subs '\d' '[[:digit:]]'
        set -l subs $subs '\D' '[^[:digit:]]'
        set -l subs $subs '\w' '[[:word:]]'
        set -l subs $subs '\W' '[^[:word:]]'
        set -l subs $subs '\l' '[[:lower:]]'
        set -l subs $subs '\L' '[^[:lower:]]'
        set -l subs $subs '\u' '[[:upper:]]'
        set -l subs $subs '\U' '[^[:upper:]]'
        set -l subs $subs '\p' '[[:print:]]'
        set -l subs $subs '\P' '[^[:print:]]'
        set -l subs $subs '\<' '[[:<:]]'
        set -l subs $subs '\>' '[[:>:]]'
        set -l subs $subs '\ms' '[:space:]'
        set -l subs $subs '\md' '[:digit:]'
        set -l subs $subs '\mw' '[:word:]'
        set -l subs $subs '\ml' '[:lower:]'
        set -l subs $subs '\mu' '[:upper:]'
        set -l subs $subs '\mp' '[:print:]'
        set -l patterns ''
        set -l substitutions ''

        for s in (seq 1 2 (count $subs))
          printf "%s => %s\n" $subs[$s] $subs[(math "$s + 1")]
          #if test (echo "$s % 2" | env BC_SCALE_MAX=0 bc) != 0
          #  echo -n $subs[$s] '=> '
          #else
          #  echo $subs[$s]
          #end
  ...
  for cc in s S w W d D l L u U p P md ms mw ml mu mp
    set result (printf '%s' (printf '%s' "$result" | sed -E (printf 's/[\]%s/\\\%s/g' $cc $$cc)))
    #echo "pat => $cc ; result => $result"
  end
